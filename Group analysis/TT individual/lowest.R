...
lowest <- data.frame(matrix(nrow=180))
endpoint <- vector()
datafit <- data.frame()
tResult <- data.frame()
...
write.csv(tResult, "TTtResult.csv", row.names = TRUE)
...
write.csv(datafit, "fitTT.csv")



dim(endpoint) <- c(length(endpoint), 1)
write.csv(endpoint, "endpointTT.csv")
write.csv(lowest, "lowestTT.csv", na="")
